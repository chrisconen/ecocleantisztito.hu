...
def main():
    ...
    p.add_argument('--owner-only',action='store_true');args=p.parse_args()
    ...
    for url in ([] if args.owner_only else SWATCHES):
        ...
        row={'source':url,'ground_truth':'manufacturer-listed NovaLife swatch',**analyze(data)}
    ...
    target=Path(__file__).resolve().parent.parent/('qa/novalife-owner-live.json' if args.owner_only else 'qa/novalife-live.json')
    target.parent.mkdir(exist_ok=True);target.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    return 0 if all(x['http_status']==200 and x.get('novalife') and not x.get('false_clearance') for x in results) else 1
