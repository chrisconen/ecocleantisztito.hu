            ...
            <div class="item-upsell" id="upsell-${fullId}" style="display:none;">
                ...
                <div class="pillow-extra">
                    <label class="upsell-text" for="pillows-${fullId}">${PILLOW_CLEANING.name}
                        ...
                        <small>Összes darabszám az itt kiválasztott bútorokhoz · ${activePillowPrice().toLocaleString('hu-HU')} Ft/db</small>
                    </label>
...
function refreshLocalPriceLabels(){
 ...
 document.querySelectorAll('[data-item-id]').forEach(card=>{
  ...
  card.querySelectorAll('.upsell-checkbox').forEach(row=>{const input=row.querySelector('input'),out=row.querySelector('.upsell-price');if(!input||!out)return;
   ...
   if(value!==undefined)out.textContent='+'+value.toLocaleString('hu-HU')+' Ft'+(extra?(extra.priceType==='perSide'?'/oldal':'/ágy'):'');
  });
  const pillow=card.querySelector('.pillow-extra .upsell-text small:last-child');if(pillow)pillow.textContent='Összes párna az itt kiválasztott bútorokhoz · '+activePillowPrice().toLocaleString('hu-HU')+' Ft/db';
 });
...
async function submitBooking(event) {
    ...
    if(isGyorCity()&&!/^gy[oő]r(?:$|[\s,\-])/iu.test(city.trim())){
        alert("A győri városi árak csak Győr városhatárán belüli címre érvényesek. Kérjük, javítsd a települést vagy a kiszállási körzetet.");
