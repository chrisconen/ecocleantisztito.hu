...
(function (global) {
  ...
  {
    "id": "karpit_ebedlo_szek",
    "label": "Kárpitozott ebédlőszék",
    "price": 4000,
    "duration": 10,
    "seats": 1,
    "image": "dining",
  ...
  {
    "id": "karpit_irodai_szek",
    "label": "Kárpitozott irodai szék",
    "price": 4500,
    "duration": 15,
    "seats": 1,
    "image": "office",
  ...
  {
    "id": "matrac_egyagyas_a",
    "label": "Egyágyas matrac · egy oldal",
    "price": 9000,
    "duration": 25,
    "sides": 1,
    "image": "mattress",
  ...
  {
    "id": "matrac_egyagyas_ab",
    "label": "Egyágyas matrac · két oldal",
    "price": 14000,
    "duration": 40,
    "sides": 2,
    "image": "mattress",
  ...
  {
    "id": "matrac_francia_a",
    "label": "Franciaágy matrac · egy oldal",
    "price": 14000,
    "duration": 35,
    "sides": 1,
    "image": "mattress",
  ...
  {
    "id": "matrac_francia_ab",
    "label": "Franciaágy matrac · két oldal",
    "price": 19500,
    "duration": 55,
    "sides": 2,
    "image": "mattress",
  ...
  const extras = {
  "karpit_folteltavolitas": {
    "label": "Extra folteltávolítás",
    "price": 2500,
    "basis": "seats",
    "duration": 10,
    "unit": "ülőhely"
  ...
  "matrac_nedves_tisztitas": {
    "label": "Folteltávolítás és fertőtlenítő nedves matractisztítás",
    "price": 3500,
    "basis": "sides",
    "duration": 20,
    "unit": "oldal",
  ...
  "matrac_agykeret": {
    "label": "Ágykeret, fejtámla tisztítása",
    "price": 4500,
    "basis": "mattresses",
    "duration": 15,
    "unit": "matrac",
  ...
  const gyorCityTariff={"karpit":{"szofa":{"price":7900},"l_kanape":{"price":17900},"u_kanape":{"price":18900},"fotel":{"price":3900},"ebedlo_szek":{"price":2400},"irodai_szek":{"price":2400}},"matrac":{"egyagyas_a":{"price":4900,"wetPrice":1900,"framePrice":3900},"egyagyas_ab":{"price":5900,"wetPrice":1900,"framePrice":3900},"francia_a":{"price":6900,"wetPrice":1900,"framePrice":3900},"francia_ab":{"price":7900,"wetPrice":1900,"framePrice":3900},"gyerek_a":{"price":2900,"wetPrice":900,"framePrice":3900},"gyerek_ab":{"price":3900,"wetPrice":900,"framePrice":3900},"kisagy_a":{"price":2400,"wetPrice":900,"framePrice":3900},"kisagy_ab":{"price":3400,"wetPrice":900,"framePrice":3900}},"pillowPrice":400};
  ...
  function calculate(input){
    ...
    s.items.forEach(i=>{const p=itemAt(i.id,s);add(i.count+' × '+p.label,p.price*i.count,i.id);duration+=p.duration*i.count;counts[p.sides?'mattresses':'upholstery']+=i.count;counts.seats+=(p.seats||0)*i.count;counts.sides+=(p.sides||0)*i.count;i.upsells.forEach(x=>{add((x==='atkairtas'?'Atkairtás':'Ágyazható felület tisztítása')+' · '+i.count+' db',p[x==='atkairtas'?'atkaPrice':'agyazhatoPrice']*i.count);duration+=(x==='atkairtas'?15:20)*i.count;});});
    s.extras.forEach(x=>{const e=extras[x];if(e.priceKey){s.items.filter(i=>i.id.startsWith('matrac_')).forEach(i=>{const p=itemAt(i.id,s),n=i.count*(e.basis==='sides'?p.sides:1);add(e.label+' · '+p.label,p[e.priceKey]*n);duration+=e.duration*n;});}else{const n=counts[e.basis];add(e.label,e.price*n);duration+=e.duration*(e.basis==='seats'?Math.ceil(n/2):n);}});
    const travelFee=s.items.length&&s.travelZone?travelAt(s.city,s.travelZone):0;if(s.items.length&&s.travelZone)add('Kiszállás · '+zones[s.travelZone].label,travelFee);
  ...
  function assertTariff(pricing,upsells,discounts,input){
    const s=validate(input),message='Az online díjak időközben változtak. Kérjük, állítsd össze újra a tételeket a megrendelőben.';
    for(const i of s.items){const sep=i.id.indexOf('_'),p=pricing[i.id.slice(0,sep)]?.[i.id.slice(sep+1)],expected=itemAt(i.id,s);
    ...
    if(s.travelZone)assert(pricing.travelZones[s.travelZone]?.fee===travelAt(s.city,s.travelZone),message);
  ...
  function mount(root){
    ...
    function update(){
      ...
      root.querySelector('.med-config-summary-status').textContent=!state.items.length?'Válaszd ki a bútorokat.':r.isLargeOrder?'Nagyobb összeállítás · egyedi időpontegyeztetés':isGyorCity(state)?(english?'Győr city prices · free travel':'Győri városi árak · díjmentes kiszállás'):state.travelZone?'Kiszállással együtt · tájékoztató összeg':'Kiszállás nélkül · válassz körzetet';
      ...
      groups.forEach(g=>{const el=root.querySelector('[data-group="'+g.id+'"]'),id=choices[g.id],p=itemAt(id,state),i=state.items.find(i=>i.id===id),im=imageFor(root,p);el.querySelector('[data-count]').textContent=(i?.count||0)+' db';el.querySelector('[data-delta="-1"]').disabled=!i;el.querySelector('[data-delta="1"]').disabled=i?.count>=99;el.querySelector('[data-unit-price]').textContent=money(p.price)+' / db';const img=el.querySelector('img');img.src=im.src;img.alt=im.alt;img.width=im.width;img.height=im.height;el.querySelectorAll('[data-item-extra]').forEach(c=>{c.disabled=!i;c.checked=!!i?.upsells.includes(c.dataset.itemExtra);c.closest('label').querySelector('small').textContent='+ '+money(p[c.dataset.itemExtra==='atkairtas'?'atkaPrice':'agyazhatoPrice'])+' / db';});});
  ...
  global.EcoStudioConfig={catalog,groups,extras,zones,cities:cityNames,excluded,validate,calculate,assertTariff,encode,decode,mount,money};
