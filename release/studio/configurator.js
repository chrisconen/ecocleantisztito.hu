...
(function (global) {
  ...
  function mount(root){
    ...
    function update(){
      ...
      const list=root.querySelector('.med-config-breakdown');list.replaceChildren();r.lines.forEach(l=>{const li=document.createElement('li');li.className='med-config-line';const span=document.createElement('span'),strong=document.createElement('strong');span.textContent=l.label;strong.textContent=money(l.amount);li.append(span,strong);list.append(li);});
