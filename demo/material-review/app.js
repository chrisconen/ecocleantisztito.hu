...
(() => {
 ...
 function novaPanel(d){
  ...
  if(state==='likely_other'){const next=el('a','Tovább az árkalkulátorhoz →','eco-material-button');next.href=root.dataset.next;actions.append(next);}
  else actions.append(choose('Címkefotót választok','label'));
  ...
  const emailAction=el('a','E-mailes ellenőrzést kérek','eco-material-button'+(state==='likely_other'?' eco-material-button-quiet':''));emailAction.href='#material-email-review';emailAction.addEventListener('click',()=>reviewEmail.focus({preventScroll:true}));actions.append(emailAction);panel.append(actions);
  const tip=el('p','A címkét gyakran az ülőlap alatt vagy a bútor alján találod. Ha megvan, a gyártó kezelési útmutatóját is készítsd elő.','eco-material-novalife-tip');
  if(state==='likely_other'){const optional=el('details',undefined,'eco-material-optional-label');optional.append(el('summary','Kezelési címkét is mutatnál? Opcionális.'),tip,choose('Címkefotót választok','label'));panel.append(optional);}else panel.append(tip);return panel;
 }
