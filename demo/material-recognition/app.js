...
(() => {
 ...
 let siteKey='',securityScript=null;
 ...
 function buttons(){
  analyze.disabled=!image||!ready||busy;root.setAttribute('aria-busy',String(busy));analyze.replaceChildren(document.createTextNode(busy?'A részleteket vizsgáljuk…':'Elemzés indítása '));if(!busy){const arrow=el('span','→');arrow.setAttribute('aria-hidden','true');analyze.append(arrow);}
 ...
 async function health(){
  ...
  try{
   ...
   collectionEnabled=r.ok&&data?.collection_enabled===true;
 ...
 function validate(data){
  ...
  clean.biztonsag=Number.isFinite(data.biztonsag)?Math.max(0,Math.min(100,Math.round(data.biztonsag))):0;
  clean.tisztitasi_kod=data.kep_tipus==='cimke'&&['W','S','WS','X'].includes(data.tisztitasi_kod)?data.tisztitasi_kod:'ismeretlen';
  if(!clean.indoklas||data.kep_tipus!=='hasznalhatatlan'&&!clean.anyag)throw Error('Az elemzésből fontos részlet hiányzik. Kérjük, próbáld újra.');return clean;
 }
 function show(data,sample=false){
  const d=validate(data);result.replaceChildren();result.hidden=false;result.dataset.sample=String(sample);
